...
class ctint_solver {
  ...
  gf_view<imfreq> Phonon_iw() { return phonon_iw; }
  gf_view<imtime> Phonon_tau() { return phonon_tau; }

  /// Construct a ctint solver
  ctint_solver(double beta_, int n_iw = 1024, int n_tau = 100001);

  /// Method that performs the QMC calculation
  void solve(double U, double alpha, double PU, double delta, int n_cycles, int length_cycle = 50, int n_warmup_cycles = 5000, std::string random_name = "",
             int max_time = -1);
};
